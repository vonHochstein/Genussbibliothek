...
(function () {
  ...
  // if (dbg) dbg.textContent = `gin id=${id}`;

  // Ziel-Element in der HTML
  const titleEl                 = document.getElementById("ginName");
  const distilleryEl            = document.getElementById("ginDistillery");
  const dotEl                   = document.getElementById("ginDot");
  const bottlerEl               = document.getElementById("ginBottler");
  ...
  const originLine              = document.getElementById("ginOriginLine");
  const imgWrapEl               = document.getElementById("ginImageWrap");
  const imgEl                   = document.getElementById("ginImage");
  const volumeEl                = document.getElementById("ginVolume");
  const abvEl                   = document.getElementById("ginAbv");
  const statsLineEl             = document.getElementById("ginStatsLine");
  const sepVolAbvEl             = document.getElementById("sepVolAbv");
  const caskLineEl              = document.getElementById("ginCaskLine");
  const caskEl                  = document.getElementById("ginCaskType");
  const priceEl                 = document.getElementById("ginPrice");
  const outturnLineEl           = document.getElementById("ginOutturnLine");
  const outturnEl               = document.getElementById("ginOutturn");
  const editBottleOutturnEl     = document.getElementById("editBottleOutturn");
  const collectorBadgeEl        = document.getElementById("ginCollectorBadge");
  const provisionalBadgeEl      = document.getElementById("ginProvisionalBadge");
  ...
  const editGinNameEl        = document.getElementById("editGinName");
  ...
  const btnDeleteGin         = document.getElementById("btnDeleteGin");
  const btnCancelMasterDataEdit = document.getElementById("btnCancelMasterDataEdit");
  const deleteGinModalEl     = document.getElementById("deleteGinModal");
  const deleteGinModalTextEl = document.getElementById("deleteGinModalText");
  const btnCancelDeleteGin   = document.getElementById("btnCancelDeleteGin");
  const btnConfirmDeleteGin  = document.getElementById("btnConfirmDeleteGin");
  ...
  function renderMasterDataView() {
    if (titleEl) {
      titleEl.textContent = savedGinName || "";
    }
    ...
    if (savedGinName) {
      document.title = `Gin – ${savedGinName}`;
      window.parent?.postMessage({ type: "gdb-gin-set-title", value: `Gin – ${savedGinName}` }, "*");
    }
  ...
  function requireUpdatePermission() {
    return window.parent?.GdbPermissions?.requirePermission?.('gin', 'update') ?? true;
  }

  function requireDeletePermission() {
    return window.parent?.GdbPermissions?.requirePermission?.('gin', 'delete') ?? true;
  }
  ...
  async function saveMasterDataToDb(nextMasterData) {
    ...
    const res = await fetch(
      `${SUPABASE_URL}/rest/v1/gdb_gins?id=eq.${encodeURIComponent(id)}`,
      {
        method: "PATCH",
        headers: {
  ...
  async function deleteGinFromDb() {
    const parentSupabase = window.parent?.supabaseClient || window.supabaseClient || null;
    ...
    const deleteUserRes = await fetch(
      `${SUPABASE_URL}/rest/v1/gdb_gin_user?gin_id=eq.${encodeURIComponent(id)}`,
      {
        method: "DELETE",
        headers: {
          apikey: SUPABASE_ANON_KEY,
    ...
    if (!deleteUserRes.ok) {
      throw new Error(`Gin-Nutzerdaten löschen fehlgeschlagen (HTTP ${deleteUserRes.status})`);
    }

    const deleteGinRes = await fetch(
      `${SUPABASE_URL}/rest/v1/gdb_gins?id=eq.${encodeURIComponent(id)}`,
      {
        method: "DELETE",
        headers: {
    ...
    if (!deleteGinRes.ok) {
      throw new Error(`Gin löschen fehlgeschlagen (HTTP ${deleteGinRes.status})`);
    }
  }

  function openDeleteGinModal(ginLabel) {
  if (!deleteGinModalEl) return;
  if (deleteGinModalTextEl) {
    deleteGinModalTextEl.innerHTML = `
      Soll „${ginLabel}“ wirklich endgültig gelöscht werden?<br><br>
  ...
  deleteGinModalEl.classList.remove("hidden");
  deleteGinModalEl.setAttribute("aria-hidden", "false");
}

function closeDeleteGinModal() {
  if (!deleteGinModalEl) return;
  deleteGinModalEl.classList.add("hidden");
  deleteGinModalEl.setAttribute("aria-hidden", "true");
}
  ...
  function getStoragePathFromPublicUrl(publicUrl) {
    ...
    try {
      ...
      const marker = "/storage/v1/object/public/gins/";
  ...
  async function saveImageToStorage() {
    ...
    if (deleteImageOnSave) {
      ...
      if (oldPaths.length) {
        const { error: cleanupError } = await parentSupabase.storage.from("gins").remove(oldPaths);
    ...
    if (!window.GdbGinImages?.createThumbnail) {
    ...
    const filePath = `gin_${id}_${timestamp}.${safeExt}`;
    const thumbnailBlob = await window.GdbGinImages.createThumbnail(pendingImageFile);
    ...
    const thumbnailPath = `thumbnails/gin_${id}_${timestamp}.${thumbnailExtension}`;
    const uploadedPaths = [];
    ...
    try {
      ...
      const publicUrl = parentSupabase.storage.from("gins").getPublicUrl(filePath).data?.publicUrl || "";
      const thumbnailUrl = parentSupabase.storage.from("gins").getPublicUrl(thumbnailPath).data?.publicUrl || "";
      ...
      if (oldPaths.length) {
        ...
        if (pathsToRemove.length) {
          const { error: cleanupError } = await parentSupabase.storage.from("gins").remove(pathsToRemove);
          if (cleanupError) throw cleanupError;
        }
      }
      ...
      if (uploadedPaths.length && !imageUrlsUpdated) {
        const { error: cleanupError } = await parentSupabase.storage.from("gins").remove(uploadedPaths);
  ...
  function applyMasterDataEditMode() {
    ...
    if (isMasterDataEditMode) {
      if (editGinNameEl) editGinNameEl.value = originalGinName || "";
    ...
    if (btnDeleteGin) {
      btnDeleteGin.hidden = isMasterDataEditMode;
      btnDeleteGin.style.display = isMasterDataEditMode ? "none" : "";
    }
...
if (btnEditMasterData) {
  btnEditMasterData.addEventListener("click", async () => {
    if (!isMasterDataEditMode) {
      ...
      const nextGinName = (editGinNameEl?.value || "").trim();
      ...
      if (!nextGinName) {
        if (masterDataEditHintEl) {
          ...
          masterDataEditHintEl.textContent = "Bitte zuerst einen Ginname eingeben";
        }
        return;
      }
      ...
      try {
        ...
        savedGinName = (savedRow?.name ?? nextGinName ?? "").toString();
        ...
        if (logDetails) {
          try {
            await writeLogEntry({
              ...
              itemType: "gin",
              itemId: id,
              itemName: savedGinName || nextGinName || null,
              details: logDetails
            });
...
if (btnDeleteGin) {
  btnDeleteGin.addEventListener("click", () => {
    if (!requireDeletePermission()) {
      return;
    }

    const ginLabel = (savedGinName || originalGinName || "dieser Gin").trim();
    openDeleteGinModal(ginLabel);
  });
}

if (btnCancelDeleteGin) {
  btnCancelDeleteGin.addEventListener("click", () => {
    closeDeleteGinModal();
  });
}

if (btnConfirmDeleteGin) {
  btnConfirmDeleteGin.addEventListener("click", async () => {
    if (!requireDeletePermission()) {
      return;
    }

    const ginLabel = (savedGinName || originalGinName || "dieser Gin").trim();
    const originalDeleteText = btnDeleteGin?.textContent || "Gin löschen";
    const originalHintText = masterDataEditHintEl?.textContent || defaultMasterDataHintText;

    closeDeleteGinModal();

    if (btnDeleteGin) btnDeleteGin.disabled = true;
    if (btnEditMasterData) btnEditMasterData.disabled = true;
    if (masterDataEditHintEl) {
      masterDataEditHintEl.hidden = false;
      masterDataEditHintEl.textContent = "Lösche Gin…";
    }

    try {
      await deleteGinFromDb();
      ...
      try {
        await writeLogEntry({
          ...
          itemId: id,
          itemName: ginLabel,
          details: "Gin gelöscht"
        });
      } catch (logErr) {
        console.error(logErr);
      }

      window.parent?.postMessage({ type: "gdb-gin-nav", view: "ginList" }, "*");
    } catch (err) {
      console.error(err);
      if (masterDataEditHintEl) {
        masterDataEditHintEl.hidden = false;
      ...
      if (btnDeleteGin) {
        btnDeleteGin.disabled = false;
        btnDeleteGin.textContent = originalDeleteText;
      }
      if (btnEditMasterData) btnEditMasterData.disabled = false;
...
if (deleteGinModalEl) {
  deleteGinModalEl.addEventListener("click", (e) => {
...
document.addEventListener("keydown", (e) => {
  ...
  if (deleteGinModalEl && !deleteGinModalEl.classList.contains("hidden")) {
    closeDeleteGinModal();
  }
});
  ...
  async function saveMyDetailsToDb(nextNotes, nextColor, nextNose, nextPalate, nextFinish, nextSummary, nextTrinkgelegenheit, nextWishlist, nextStock, nextRating) {
    ...
    const res = await fetch(
      `${SUPABASE_URL}/rest/v1/gdb_gin_user?on_conflict=gin_id,user_id`,
      {
        method: "POST",
        headers: {
          apikey: SUPABASE_ANON_KEY,
  ...
  window.parent?.postMessage({ type: "gdb-gin-set-title", value: "Gin – Detailkarte" }, "*");
  ...
  async function loadGinName() {
    try {
      const accessToken = await getAccessToken();
      const url =
        `${SUPABASE_URL}/rest/v1/gdb_gins` +
        `?id=eq.${encodeURIComponent(id)}` +
        `&select=name,distillery,bottler,country,region,flag_url,region_flag_url,image_url,thumbnail_url,volume_ml,abv,fasstyp,price_eur,price_per_liter_eur,bottle_outturn,provisional,collector,created_at,updated_at,created_by,updated_by`;
      const res = await fetch(url, {
        headers: {
          apikey: SUPABASE_ANON_KEY,
          Authorization: `Bearer ${accessToken}`,
      ...
      const ginCreatedAt = rows?.[0]?.created_at || "";
      const ginUpdatedAt = rows?.[0]?.updated_at || "";
      const ginCreatedBy = rows?.[0]?.created_by || "";
      const ginUpdatedBy = rows?.[0]?.updated_by || "";
      ...
      if (cEl) cEl.textContent = fmtDE(ginCreatedAt);
      if (uEl) uEl.textContent = fmtDE(ginUpdatedAt);

      try {
        const userMap = await fetchUserDisplayMap([ginCreatedBy, ginUpdatedBy]);
        if (createdByEl) createdByEl.textContent = ginCreatedBy ? (userMap[ginCreatedBy] || "–") : "–";
        if (updatedByEl) updatedByEl.textContent = ginUpdatedBy ? (userMap[ginUpdatedBy] || "–") : "–";
      } catch (userErr) {
        console.error(userErr);
        if (createdByEl) createdByEl.textContent = ginCreatedBy ? ginCreatedBy : "–";
        if (updatedByEl) updatedByEl.textContent = ginUpdatedBy ? ginUpdatedBy : "–";
      }
