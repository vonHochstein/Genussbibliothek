(function () {
  ...
  function requireCreatePermission() {
    return window.parent?.GdbPermissions?.requirePermission?.('gin', 'create') ?? true;
  }
  ...
  async function saveImageToStorage(newId, thumbnailBlob) {
    ...
    const originalPath = `gin_${newId}_${timestamp}.${ext}`;
    ...
    const thumbnailPath = `thumbnails/gin_${newId}_${timestamp}.${thumbnailExtension}`;
    const uploadedPaths = [];
    ...
    try {
      ...
      const originalUrl = parentSupabase.storage.from('gins').getPublicUrl(originalPath).data?.publicUrl || '';
  ...
  async function cleanupUploadedImages(paths) {
    ...
    const { error } = await parentSupabase.storage.from('gins').remove(paths);
  ...
  function buildPayload() {
    ...
    return {
      ...
      name: (editGinNameEl?.value || '').trim() || null,
  ...
  async function saveGin() {
    ...
    if (!payload.name) {
      setHint('Bitte zuerst einen Ginname eingeben', 'err');
      return;
    }
    ...
    try {
      ...
      if (pendingImageFile && !deleteImageOnSave) {
        ...
        thumbnailBlob = await window.GdbGinImages.createThumbnail(pendingImageFile);
      }
      ...
      const accessToken = await getAccessToken();
      const res = await fetch(`${SUPABASE_URL}/rest/v1/gdb_gins`, {
        method: 'POST',
        headers: {
          apikey: SUPABASE_ANON_KEY,
          Authorization: `Bearer ${accessToken}`,
      ...
      try {
        await writeLogEntry({
          ...
          itemType: 'gin',
          itemId: newId,
          itemName: payload.name || data?.[0]?.name || 'Unbekannter Gin',
      ...
      window.location.href = `gdb_gin_detail.html?id=${encodeURIComponent(newId)}&t=${Date.now()}`;
    } catch (err) {
      console.error(err);
  ...
  function init() {
    document.title = 'Gin – Neuer Eintrag';
    window.parent?.postMessage({ type: 'gdb-gin-set-title', value: 'Gin – Neuer Eintrag' }, '*');
    if (!requireCreatePermission()) {
      btnSave.disabled = true;
      setHint('Keine Berechtigung zum Anlegen neuer Gin-Einträge.', 'err');
    }
    if (imgEl) {
    ...
    btnCancel?.addEventListener('click', goBackToGinList);
    btnSave?.addEventListener('click', saveGin);
  }
