// ==========================================================
    // 1) SUPABASE KONFIG (aus alter Ginbibliothek übernommen)
    // ==========================================================
    ...
    let MY_STOCK_BY_ID = new Map();       // gin_id -> stock (ml) vom eingeloggten User
    let OTHERS_STOCK_BY_ID = new Map();   // gin_id -> stock-Summe (ml) aller anderen

    // User-spezifische Daten (Rating) werden nachgeladen und gemappt
    let MY_RATING_BY_ID = new Map();
    let AVG_RATING_BY_ID = new Map();   // gin_id -> avg (0..10)
    let CNT_RATING_BY_ID = new Map();   // gin_id -> count
    ...
    // Alle Bestände pro Gin (für Infobox)
    let STOCKS_BY_ID = new Map();      // gin_id -> [{ user_id, stock }]
    let GIN_VOL_BY_ID = new Map();  // gin_id -> volume_ml (Number)

    // Bewertungen pro Gin (für Infobox)
    let RATINGS_BY_ID = new Map(); // gin_id -> Array<{ user_id, rating }>
    let USER_NAME_BY_ID = new Map(); // user_id -> display_name

    // Gin-Name nach ID (für Parent-Popup-Titel)
    let GIN_NAME_BY_ID = new Map(); // gin_id -> name

    // PLI Min/Max Index (für Skala in Gin Karte)
    let PLI_MIN = null;
    let PLI_MAX = null;
    ...
    // gin_id -> { created_at, updated_at }
    let GIN_META_BY_ID = new Map();

    // gin_id -> created_at / updated_at (für Detail-View)
    let GIN_CREATED_AT_BY_ID = new Map();
    let GIN_UPDATED_AT_BY_ID = new Map();

    // gin_id -> created_by / updated_by (für Detail-View)
    let GIN_CREATED_BY_BY_ID = new Map();
    let GIN_UPDATED_BY_BY_ID = new Map();

    // gin_id -> my_created_at / my_updated_at (für Detail-View)
    let MY_CREATED_AT_BY_GIN_ID = new Map();
    let MY_UPDATED_AT_BY_GIN_ID = new Map();
    ...
    function notifyParentPopupOpen(){
      window.parent?.postMessage({ type: "gdb-gin-popup-open" }, "*");
    }

    function notifyParentPopupClose(){
      window.parent?.postMessage({ type: "gdb-gin-popup-close" }, "*");
    }
    ...
    function notifyParentGinCount(count) {
      window.parent?.postMessage({ type: "gdb-gin-count", count }, "*");
    }
    ...
    function rerenderCurrentList() {
      const nextList = getFilteredAndSortedList(ALL_GINS);
      renderList(nextList);
      notifyParentGinCount(nextList.length);
    }    
...
function renderList(list){
  ...
  content.innerHTML = list.map(w => {
    ...
    // Bestand-Balken:
    // - mein Bestand: gdb_gin_user.stock für CURRENT_UID (ml)
    // - sonstige Bestände: Summe gdb_gin_user.stock aller anderen (ml)
    ...
    return `
      <div class="card ${cardClass}" data-gin-id="${w.id}">
        <div class="row gin-row">
          <div class="gin-img-col">
            <div class="gin-img-wrap">
              <img loading="lazy" decoding="async" src="${imgSrc}" class="gin-img" alt="">
            </div>
    ...
    (function initPopupBridge(){
      ...
      document.addEventListener("click", (e) => {
        ...
        window.parent?.postMessage({ type: "gdb-gin-open-pli" }, "*");
      });
      ...
      document.addEventListener("click", (e) => {
        ...
        const ginName = GIN_NAME_BY_ID.get(id) || "";
        window.parent?.postMessage({
          type: "gdb-gin-open-ratings",
          ginId: id,
          title: ginName
            ? `Bewertungen<br><small>${escapeHtml(ginName)}</small>`
            : "Bewertungen",
          html
        }, "*");
      });
    ...
    document.addEventListener("click", (e) => {
      ...
      const ginId = btn.getAttribute("data-stock-info");
      if (!ginId) return;

      const rows = STOCKS_BY_ID.get(ginId) || [];
      const vol = GIN_VOL_BY_ID.get(ginId) || 0;
      ...
      const ginName = GIN_NAME_BY_ID.get(ginId) || "";
      window.parent?.postMessage({
        type: "gdb-gin-open-stocks",
        ginId,
        title: ginName
          ? `Bestände<br><small>${escapeHtml(ginName)}</small>`
          : "Bestände",
        html
      }, "*");
    });
...
async function boot(){
  ...
  const content = document.getElementById("content");
  content.innerHTML = "<p class='muted'>Lade Gins …</p>";
  ...
  GIN_VOL_BY_ID = new Map((data || []).map(w => [w.id, Number(w.volume_ml ?? 0)]));

  // Wir definieren ids EINMAL sauber (wird mehrfach gebraucht)
  const ids = (data || []).map(w => w.id);
  GIN_CREATED_AT_BY_ID = new Map((data || []).map(w => [w.id, w.created_at || ""]));
  GIN_UPDATED_AT_BY_ID = new Map((data || []).map(w => [w.id, w.updated_at || ""]));
  GIN_CREATED_BY_BY_ID = new Map((data || []).map(w => [w.id, w.created_by || ""]));
  GIN_UPDATED_BY_BY_ID = new Map((data || []).map(w => [w.id, w.updated_by || ""]));
  ...
  const myRowsPromise = (CURRENT_UID && ids.length)
    ? supabase
        .from("gdb_gin_user")
        .select("gin_id, stock, rating, pli, notes, t_color, t_nose, t_palate, t_finish, t_summary, t_trinkgelegenheit, wishlist, created_at, updated_at")
  ...
  if (CURRENT_UID && ids.length) {
    ...
    if (!myErr && myRows) {
      for (const r of myRows) {
        MY_STOCK_BY_ID.set(r.gin_id, Number(r.stock ?? 0));
        MY_RATING_BY_ID.set(
          r.gin_id,
        ...
        MY_NOTES_BY_ID.set(r.gin_id, (r.notes ?? "").toString());
        MY_COLOR_BY_ID.set(r.gin_id, (r.t_color ?? "").toString());
        MY_NOSE_BY_ID.set(r.gin_id, (r.t_nose ?? "").toString());
        MY_PALATE_BY_ID.set(r.gin_id, (r.t_palate ?? "").toString());
        MY_FINISH_BY_ID.set(r.gin_id, (r.t_finish ?? "").toString());
        MY_SUMMARY_BY_ID.set(r.gin_id, (r.t_summary ?? "").toString());
        MY_TRINKGELEGENHEIT_BY_ID.set(r.gin_id, (r.t_trinkgelegenheit ?? "").toString());
        MY_WISHLIST_BY_ID.set(r.gin_id, !!r.wishlist);
        MY_CREATED_AT_BY_GIN_ID.set(r.gin_id, r.created_at || "");
        MY_UPDATED_AT_BY_GIN_ID.set(r.gin_id, r.updated_at || "");
      }
    }
    ...
    if (!stockAllErr && stockRowsAll) {
      ...
      OTHERS_STOCK_BY_ID = new Map();
      for (const [ginId, rows] of STOCKS_BY_ID.entries()) {
        let sum = 0;
        for (const r of rows) {
          if (r.user_id === CURRENT_UID) continue;
          sum += Number(r.stock ?? 0);
        }
        OTHERS_STOCK_BY_ID.set(ginId, sum);
      }
    }
  } else {
  ...
  GIN_NAME_BY_ID = new Map((data || []).map(w => [w.id, w.name || ""]));

  GIN_META_BY_ID = new Map((data || []).map(w => [w.id, {
  ...
  console.timeEnd("boot_total");
    ...
    window.addEventListener("message", (e) => {
      ...
      if (e.data.type === "gdb-gin-popup-open") {
        overlay.classList.add("active");
        overlay.setAttribute("aria-hidden", "false");
      }

      if (e.data.type === "gdb-gin-popup-close") {
        overlay.classList.remove("active");
        overlay.setAttribute("aria-hidden", "true");
      }

      if (e.data.type === "gdb-gin-set-search") {
      ...
      if (e.data.type === "gdb-gin-set-compare-stock") {
...
document.addEventListener("click", (e) => {
  ...
  const card = e.target.closest(".card[data-gin-id]");
  if (!card) return;

  const ginId = card.getAttribute("data-gin-id");
  if (!ginId) return;

  const avgRating = AVG_RATING_BY_ID.get(ginId);
  const cntRating = CNT_RATING_BY_ID.get(ginId) || 0;

  const avgPli = AVG_PLI_BY_ID.get(ginId);
  const cntPli = CNT_PLI_BY_ID.get(ginId) || 0;

  const pliMin = PLI_MIN;
  const pliMax = PLI_MAX;

  const myStockMl = MY_STOCK_BY_ID.get(ginId) || 0;
  const myRating = MY_RATING_BY_ID.get(ginId);
  const myPli    = MY_PLI_BY_ID.get(ginId);

  const myNotes = MY_NOTES_BY_ID.get(ginId) || "";
  const myColor = MY_COLOR_BY_ID.get(ginId) || "";
  const myNose  = MY_NOSE_BY_ID.get(ginId) || "";
  const myPalate = MY_PALATE_BY_ID.get(ginId) || "";
  const myFinish = MY_FINISH_BY_ID.get(ginId) || "";
  const mySummary = MY_SUMMARY_BY_ID.get(ginId) || "";
  const myTrinkgelegenheit = MY_TRINKGELEGENHEIT_BY_ID.get(ginId) || "";
  const myWishlist = MY_WISHLIST_BY_ID.get(ginId) ? 1 : 0;

  const createdById = GIN_CREATED_BY_BY_ID.get(ginId) || "";
  const updatedById = GIN_UPDATED_BY_BY_ID.get(ginId) || "";
  ...
  // console.log("NAV_PAYLOAD", ginId, avgPli, cntPli);
  console.log("NAV payload test", { ginId, myColor: (MY_COLOR_BY_ID?.get(ginId) ?? null) });
  ...
  window.parent.postMessage(
  {
    ...
    created_at: GIN_CREATED_AT_BY_ID.get(ginId) || "",
    updated_at: GIN_UPDATED_AT_BY_ID.get(ginId) || "",

    created_by: createdByName,
    updated_by: updatedByName,

    my_created_at: MY_CREATED_AT_BY_GIN_ID.get(ginId) || "",
    my_updated_at: MY_UPDATED_AT_BY_GIN_ID.get(ginId) || "",
